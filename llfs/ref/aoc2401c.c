...
long similarity_score(int *left, int *right, int entries) {
	sort(left, entries);
	long sum = left[0] * count_occurences(left[0], right, entries);
	for (int i = 1; i < entries; ++i)
		if (left[i] != left[i-1]) // this works because we never use i==0 here!
...
// same result as b, but only because there are no duplicates on the left side
